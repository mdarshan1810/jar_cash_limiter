...
class KotakFixtureSuiteTest {

    private val parser = KotakParser()
    private val json = Json { ignoreUnknownKeys = true }

    private fun samplesDir(): File {
        val url = javaClass.classLoader?.getResource("kotak_samples")
            ?: error("kotak_samples/ not on classpath — check src/test/resources")
    ...
    /**
     * Same comparison shape as HdfcFixtureSuiteTest. Kept inline rather than extracted to a
     * shared helper because the two banks may diverge later (e.g. bank-specific assertions).
     */
